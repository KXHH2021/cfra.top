---
title: YOLOV8优化(4), yolov8改进添加非极大值抑制(soft-nms)
date: 2024-2-4 15:38:56
categories:
  - YOLOV8优化
tags:
  - YOLOV8
...
有时候我们进行推理时检测框太多了，通常我们会调整`conf`值来抑制检测框的出现，但当`conf`值要求固定时，我们需要加入nms（非极大值抑制）来限制检测框的出现。接下来教大家如何改进添加非极大值抑制(soft-nms)。

首先需要修改损失函数部分

在`ultralytics/yolo/utils/metrics.py`加入以下代码：
...
在`ultralytics/yolo/utils/ops.py`中找到 `non_max_suppression` 函数，
把原`torchvision.ops.nms`替换。

注意：在`ops.py`需要导入`soft-nms`

`from .metrics import box_iou,soft_nms`

原`torchvision.ops.nms`为：
